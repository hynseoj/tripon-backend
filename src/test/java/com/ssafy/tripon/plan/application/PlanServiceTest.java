...
class PlanServiceTest {
    ...
    void canSavePlan() {
        ...
        // when
        Integer savedId = planService.savePlan(command);

        // then
        PlanServiceResponse response = planService.findPlanById(savedId);
        assertThat(response.title()).isEqualTo("제주 여행");
    }

    @DisplayName("사용자 ID로 계획 목록을 조회할 수 있다")
    @Test
    void canGetPlanListByMemberId() {
        // given
    ...
    void canFindPlanById() {
        ...
        // when
        PlanServiceResponse plan = planService.findPlanById(savedId);

        // then
        assertThat(plan.memo()).isEqualTo("부산 메모");
    }
    ...
    void canUpdatePlan() {
        // given
        Integer savedId = planService.savePlan(new PlanSaveCommand("admin@ssafy.com", "오래된 여행", LocalDate.now(), LocalDate.now().plusDays(1), ""));
        PlanUpdateCommand updateCommand = new PlanUpdateCommand(
                savedId, "업데이트된 여행", LocalDate.now(), LocalDate.now().plusDays(4), "업데이트 메모"
        );
        ...
        // then
        PlanServiceResponse updated = planService.findPlanById(savedId);
        assertThat(updated.title()).isEqualTo("업데이트된 여행");
        assertThat(updated.memo()).isEqualTo("업데이트 메모");
    }
    ...
    void canDeletePlan() {
        // given
        Integer savedId = planService.savePlan(new PlanSaveCommand("admin@ssafy.com", "삭제할 여행", LocalDate.now(), LocalDate.now().plusDays(1), ""));

        // when
        planService.deletePlanById(savedId);

        // then
        List<PlanServiceResponse> plans = planService.findAllPlanByMemberId("admin@ssafy.com");
        assertThat(plans).noneMatch(plan -> plan.planId().equals(savedId));
    }
}
